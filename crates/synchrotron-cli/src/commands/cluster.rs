...
pub async fn run(client: &SynchrotronClient, format: OutputFormat, cmd: ClusterCmd) -> Result<()> {
    match cmd {
        ...
        ClusterCmd::Create(a) => {
            ...
            if let Some(x) = a.kubeconfig_path {
                body.insert("kubeconfig_path".into(), json!(x));
            }
            if let Some(x) = a.context {
                body.insert("context".into(), json!(x));
            }
            if let Some(x) = a.bearer_token {
                body.insert("bearer_token".into(), json!(x));
            }
        ...
        ClusterCmd::Update(a) => {
            let mut body = serde_json::Map::new();
            if let Some(x) = a.auth_source {
                body.insert("auth_source".into(), json!(x));
            }
            if let Some(x) = a.kubeconfig_path {
                body.insert("kubeconfig_path".into(), json!(x));
            }
